...
const Canvas = () => {
  ...
  //결과 list
  const [keyResultParseList, setKeyResultParseList] = useState<
    ResultInterface[]
  >([]);
  //parse list, filled된 parse list
  ...
  const [createdLabel, setCreatedLabel] = useState<ParseLabel>(
    new ParseLabel()
  );
  //popup 띄우는지
  ...
  //signin popup 띄우는지
  const [isSigninShown, setIsSigninShown] = useState<boolean>(false);

  //signup popup 띄우는지
  const [isSignupShown, setIsSignupShown] = useState<boolean>(false);

  //toast
  const successFileToast = useToast(
    "success",
    "파일들을 성공적으로 가져왔습니다",
    "닫기",
    () => {}
  );
  const successImportToast = useToast(
    "success",
    "성공적으로 import했습니다",
    "닫기",
    () => {}
  );
  const successExportToast = useToast(
    "success",
    "성공적으로 export했습니다",
    "닫기",
    () => {}
  );
  const successSaveToast = useToast(
    "success",
    "성공적으로 저장했습니다",
    "닫기",
    () => {}
  ...
  const setGeneralResultParseList = (
    obj: ResultInterface[],
    index?: number
  ) => {
    var realIndex = index ?? tabIndex;
  ...
  const getDataFromJson = async (
    ...
    setParseData(newPoints, newLabelPoints, tmpTabIndex);

    //resultParseList에도 저장
    const generalResultParseList2 = generalResultParseList(tmpTabIndex);
    setGeneralResultParseList(
      [
        ...generalResultParseList2.slice(0, fileIndex),
        {
          points: newPoints,
  ...
  const getOneDataFromJson = async (file: File, fileIndex: number) => {
    ...
    jsonData["shapes"].map(
      (item: { color?: string; points: number[][]; label: string }) => {
        let color = item["color"];
        if (!color) {
          //make random color
          color = "#" + Math.floor(Math.random() * 16777215).toString(16);
  ...
  const saveToResultList = () => {
    setResultParseList([
      ...resultParseList.slice(0, imageIndex),
      {
        points: points,
    ...
    //keypoints
    setKeyResultParseList([
      ...keyResultParseList.slice(0, imageIndex),
      {
        points: keyPoints,
  ...
  const getMousePos = (stage: StageType) => {
    //마우스 position 가져오기
    return [
      stage.getPointerPosition()!.x / ratio,
      stage.getPointerPosition()!.y / ratio,
    ];
  };
  ...
  const handleMouseDown = (e: KonvaEventObject<MouseEvent>) => {
    ...
    if (
      work !== "create" ||
      (curIndex !== -1 && tabIndex === 0 && isPolyComplete[curIndex])
    )
      return;
    else if (curIndex === -1) {
      //create
    ...
    if (
      isGeneralMouseOverPoint()[curIndex] &&
      generalPoints()[curIndex].length >= 3
    ) {
      ...
      const newFlattenPoints = generalFlattenedPoints().map((item, i) => {
        if (i === curIndex) {
          return generalPoints2[curIndex].reduce((a, b) => a.concat(b), []);
        }
        return item;
      });
  ...
  const handleMouseOverStartPoint = (
    ...
    if (tabIndex === 0 && (isPolyComplete[index] || points[index].length < 3))
      return;
    //원 크게 보이도록
      ...
      const newFlattenPoints = generalFlattenedPoints().map((item, index) => {
        if (curIndex === index) {
          return generalPoints()
            [index].concat(isGeneralPolyComplete()[index] ? [] : position)
      ...
      const newPoints = generalPoints().map((item, i) => {
        if (i === index) {
          ...
          copyPoints.map((point) =>
            result.push([
              point[0] + e.target.x() / ratio,
              point[1] + e.target.y() / ratio,
            ])
          );
          return result;
        }
    ...
    <div>
      <div className="wrapper">
              ...
              {tabIndex === 0 &&
                points.map((item: number[][], index: number) => (
                  <PolygonAnnotation
                    ...
                    originalPoints={item}
                    flattenedPoints={flattenedPoints[index].map(
                      (x) => x * ratio
                    )}
                    changePoints={changePoints}
                    deletePolygon={deletePolygon}
                    handlePointDragMove={(e: KonvaEventObject<DragEvent>) =>
                      handlePointDragMove(e, index)
                    }
                    handleGroupDragEnd={(e: KonvaEventObject<DragEvent>) =>
                      handleGroupDragEnd(e, index)
                    }
                    ...
                    handleMouseOutStartPoint={(
                      e: KonvaEventObject<MouseEvent>
                    ) => handleMouseOutStartPoint(e, index)}
        ...
        <div className="wrapper-menu">
          ...
          <div className="wrapper-box1">
            <div className="wrapper-title">File List</div>
          ...
          {/* Labeling > work list */}
          <div
            className="wrapper-box2"
            style={{
              display: windowIndex === 0 ? "" : "none",
            }}
          >
            <div className="wrapper-title">Work List</div>

            <div className="wrapper-tab">
              <div
                className={`wrapper-tabmenu ${
                  tabIndex === 0 ? "wrapper-tabmenu-clicked" : ""
                }`}
                onClick={() => {
                  setTabIndex(0);
                }}
              ...
              <div
                className={`wrapper-tabmenu ${
                  tabIndex === 2 ? "wrapper-tabmenu-clicked" : ""
                }`}
                onClick={() => {
                  setTabIndex(2);
                }}
          ...
          <div className="wrapper-bottom-tab">
            <div
              className={`click wrapper-bottom-tabmenu ${
                windowIndex === 0 ? "wrapper-bottom-tabmenu-clicked" : ""
              }`}
              onClick={() => {
                setWindowIndex(0);
              }}
      ...
      <div className="left-menu">
        ...
        <input
          ...
          onChange={async (e) => {
            ...
            for (let i = 0; i < filesLen; i++) {
              ...
              if (curFile.type.search("image") != -1) {
                ...
                imageFileNameList.push(
                  curFile.webkitRelativePath.split(".")[0]
                );
                imageFilesLen++;
                ...
                if (curFileName.includes("_p.json")) {
                  ...
                  parseJsonFileNameList.push(
                    curFile.webkitRelativePath.split("_p.")[0]
                  );
                } else if (curFileName.includes(".json")) {
                  //keypoints
                  keyJsonFileList.push(curFile);
                  keyJsonFileNameList.push(
                    curFile.webkitRelativePath.split(".")[0]
                  );
                }
              }
            ...
            for (let j = 0; j < parseJsonFileNameList.length; j++) {
              const matchIndex = imageFileNameList.indexOf(
                parseJsonFileNameList[j]
              );
              if (matchIndex !== -1) {
                const result: {
                  points: number[][][];
            ...
            for (let j = 0; j < keyJsonFileNameList.length; j++) {
              const matchIndex = imageFileNameList.indexOf(
                keyJsonFileNameList[j]
              );
              if (matchIndex !== -1) {
                const result = await getOneDataFromJson(
                  keyJsonFileList[j],
                  matchIndex
                );
                setParseData(
        ...
        <label htmlFor="multi_file_upload" className="click form-click">
          <IconButton iconName="icon-upload" label={"Upload"} status={false} />
        </label>
      ...
      {/* 로그인 관련 팝업 */}
      {isSigninShown && <SigninPopUp key="signin" onClose={setIsSigninShown} />}
      {isSignupShown && <SignupPopUp key="signup" onClose={setIsSignupShown} />}
    </div>
  );
};
